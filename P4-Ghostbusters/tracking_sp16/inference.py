...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        # update belief for every possible ghost position given the evidence
        for ghostPos in ghostPositions:
            probGivenEvidence=self.getObservationProb(noisyDistance, pacmanPos, ghostPos, jailPos)
            self.beliefs[ghostPos] = probGivenEvidence*self.beliefs[ghostPos]
    ...
    def observeUpdate(self, observation, gameState):
        ...
        if weightedDist.total() == 0:
            ...
            for i in range(self.numParticles):
                newSample = weightedDist.sample()
                self.particles.append(newSample)
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        while self.numParticles > len(permutations):
            self.particles += permutations
            self.numParticles -= len(permutations)
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for ghostPos in self.particles:
            ...
            for i in range(self.numGhosts):
                ...
                evidence *= self.getObservationProb(observation[i], pacmanPos, ghostPos[i], jailPos)
            weighted_distribution[ghostPos] += evidence
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            for i in range(self.numGhosts):
                prevGhostPos = oldParticle
                newPosDist = self.getPositionDistribution(gameState, prevGhostPos, i, self.ghostAgents[i])
                newPos = newPosDist.sample()
